...
export default function SessionGate({
  sessionId, children, title = '세션 인증',
}: { sessionId: string; children: React.ReactNode; title?: string; }) {
  ...
  const askPin = useCallback(() => {
    ...
    return new Promise<string>((resolve, reject) => {
      resolverRef.current.ok = (v: string) => { setOpen(false); resolve(v); };
      resolverRef.current.cancel = () => { setOpen(false); reject(new Error('cancel')); };
    });
  }, []);

  const onConfirm = () => { resolverRef.current.ok?.(inputRef.current?.value.trim() || ''); };
      ...
      {open && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.35)', display:'flex', alignItems:'center', justifyContent:'center' }}>
          <div style={{ background:'#fff', padding:20, borderRadius:12, width:320 }}>
            ...
            <input ref={inputRef} type="password" placeholder="PIN"
